  ...
  return (
    <div>
      ...
      <form className="w-3/12 absolute p-12 bg-black m-36 mx-auto right-0 left-0 text-white rounded-lg bg-opacity-80">
        <h1 className="font-bold text font-3xl py-4">{isSignInForm ? "Sign In": "Sign Up"}</h1>
        ...
        <p className="py-4 cursor-pointer" onClick={toggleSignInForm}>{isSignInForm ? "New to Netflix? Sign Up Now": "Already a user! Sign In With Your Credentials "}</p>
      </form>
    </div>
  );
};
